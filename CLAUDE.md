...
└── projects/<slug>/            # ⚠ user data — never auto-deleted
    ...
    │   │                      #   + colmap_check.json — what the COLMAP export
    │   │                      #   actually produced, and what this RealityScan
    │   │                      #   install registers for it (§7.2)
    ...
    ├── lfs_output/             # the trained splat, checkpoints/, metrics.csv
    │                          #   + eval_step_<N>/ when the run saved its
    │                          #   evaluation images (§7.6)
    ├── export/
...
is `segment`, not for it to be left unsupervised. **`ignore` is therefore not
offered at all** — it is off the `Literal`, off the radio group and out of
`defaults.json`, which now ships `segment`. A stored `"ignore"` is read as
`segment` rather than refused, so an existing project row and an older
`defaults.json` keep loading.
...
### 7.6 The evaluation pass (step 4)

Two switches, in step 4's Advanced panel and in the setup panel's LichtFeld
section, and they answer two different questions.

**Eval mode** (`--eval`) makes LichtFeld Studio hold a validation split out of
the dataset — `Created train/val split: {} train, {} val images`, at the
cadence of `--test-every N` ("Use every Nth image as test"), which the app does
**not** send, so the build's own default stands. Each evaluation prints the one
line of a run that carries PSNR — the format, read off the binary, is
`[Evaluation at step N] PSNR: …, SSIM: …, Time: …s/image, #GS: …` — which is
why the step 4 chart's PSNR series is empty for every run without it (§12,
2026-08-23). `lfs_output/metrics.csv` and `metrics_report.txt` are written
either way; `--eval` is what puts rows in them. The header-only `metrics.csv`
sitting in most projects here is exactly what a run *without* it produces.

The cost is the split itself. The held-out images are not trained on, so eval
is for comparing settings against a number, not for the run whose splat you
intend to ship.

**Save eval images** decides whether each evaluation also writes the pictures
it scored — ground truth next to render, one PNG per validation camera — under
`lfs_output/eval_step_<N>/` (`Saved {} evaluation images to: …`). The metrics
say a run got worse; the images say where.

**The flag is a negative and the build's default is on**, which is the only
part of this worth misreading. v0.5.3's own help, read off `lfs_core.dll`:
*"Disable saving of evaluation comparison images (GT vs rendered) during eval
(default: enabled)"*. So `build_lfs_command` emits `--no-save-eval-images` when
the toggle is **off**, and only inside the `if lfs.eval:` branch — the flag
disables nothing on a run that never evaluates, which is why step 4's Advanced
panel greys the switch out until Eval mode is on (the setup panel's copy of it
does not). Both keys ship `false` in `defaults.json`.

Nothing downstream sees any of it: step 5 copies `*.ply`/`*.splat` only, so
eval images never reach `export/`, and a re-training resets `lfs_output/` with
them in it (§14.1).
...
| 2026-08-26 | **The mask files only win once the alpha is switched off, and the alpha RealityScan writes is a constant.** The mask route of 7.5 was built, measured and declared working, and every training run since had been unmasked. LichtFeld Studio v0.5.3 turns alpha-as-mask on **automatically** for any RGBA image (`--no-alpha-as-mask` is documented as *disabling* it), the two mask sources are mutually exclusive, and the alpha outranks the files — the trainer logs `Alpha-as-mask enabled` *or* `Mask file loading enabled`, never both. RealityScan's COLMAP exporter writes 4-channel PNGs whatever `undistortImagesWicPixlFormat` asks for (`defaults.json` has asked for `24-bit BGR` since the export was written), and that channel is a flat 255: measured on `publicsemples_truck`, 251/251 exported images RGBA, alpha min = max = 255, one unique value, on a project whose source frames were **JPEG and had no alpha at all**. Thresholded at 0.5 that means "keep every pixel", so the run trained on the full frame while `Found 251 masks` scrolled past and `/api/files/{id}/masks` reported `state: "ready"` — every indicator the app has said the masks were fine, because each of them was answering a question about the *files*. The naming was never the fault and was checked rather than assumed: `masks/00000.png` beside `images/00000.png` is what LFS pairs, all 251 matched, and moving the folder away makes it fail with `Mask mode enabled but no masks found in <d>/masks/` — the dataset's own layout is the contract, which is the whole point of 7.5 exporting the masks *through* the COLMAP writer. Two fixes. `build_lfs_command` sends `--no-alpha-as-mask` alongside `--mask-mode` whenever the dataset carries mask files, so the files take the precedence they were exported to have. And `rc_alpha.export_keeps_alpha` **decodes** the channel over the first, middle and last image instead of reading the PNG colour-type byte: a header-only answer reported the alpha "carried" for every export RS has ever made here, which is the latent half of the same bug — an imported RGBA set (6.7) would have been told its alpha survived, skipped the `masks/` delivery, and trained against a flat 255 with nothing on screen to say so. **A mask is a supervision gate, not an eraser**: `--mask-mode ignore` drops masked pixels from the loss and deletes no gaussian, so a background the sparse cloud already initialised stays. Three 13 000-iteration runs on one dataset, gaussians inside the validated region box: unmasked **79.3 %**, masked with `ignore` **79.0 %**, masked with `segment` **96.3 %** — and of the gaussians above opacity 0.5, 60.6 % / 58.9 % / **98.9 %**, with the 99th-percentile radius falling 149.6 → 147.0 → **19.5**. So reading the masks correctly is necessary and does nothing on its own; `segment` is where the background actually goes. `defaults.json` ships `mask_mode: "ignore"`, which is the wrong default for the route 7.5 exists to serve — someone who draws a region and generates masks from it wants the background gone, not unsupervised. Left as a settings question rather than changed under the user; **settled the same day, see the row below.** **The frame is what makes any of this countable, and the first count was wrong**: `region.json` and `pointcloud.ply` are NeRF-frame (the box reproduces its stored `coverage` to the point, 44 253 / 57 482), but the COLMAP-trained splat sits at `Rx180` from them — 28.6 % occupancy-cell overlap against 2.7 % at identity — as `scene_rotate_x_deg = 180` implies. Counting without that rotation reports ~14 % for every run and hides the `segment` result entirely. |
| 2026-08-26 | **`ignore` is removed from the mask modes, and `segment` is the default.** It is the only setting in the app whose two ends measure the same: the table above puts `--mask-mode ignore` at 79.0 % of gaussians inside the region box against **79.3 %** for a run with no masks at all, 58.9 % against 60.6 % above opacity 0.5, p99 radius 147.0 against 149.6 — three columns, all within noise, because `ignore` drops the masked pixels from the loss and deletes nothing that is already there. A user who has drawn a Reconstruction Region, run the mask generation of §7.5 and then chosen the option whose label says the masked pixels are ignored gets a splat indistinguishable from the unmasked one, with every indicator in the app reporting the masks as read. An option that cannot be told from `Off` is worse than a missing one, so it is gone from `LFSDefaults`' `Literal`, from `MaskMode`, from the radio group in `LFSSettings` (which drives both the setup panel and step 4's Advanced panel — one component) and from `defaults.json`, whose `mask_mode` becomes `segment`. `segment_and_ignore` stays: it segments *and* ignores, so it is not the same option. The removal is a **coercion, not a rejection** — a `field_validator(mode="before")` reads a stored `"ignore"` as `"segment"`, because the value sits in `Project.settings_json` rows and in the `defaults.json` on the staging PC, and `resolve_lfs_settings` filters unknown *keys* but not unknown *values*: a plain `Literal` narrowing would have made `load_defaults()` raise on a file it wrote itself. |
| 2026-08-26 | **Step 3 says what this RealityScan install registers for the COLMAP export, on every run, and writes down what the export produced.** On the staging workstation the COLMAP export had *never* worked: all five projects there hold a `<slug>_COLMAP/colmap.txt` that is a plain list of the **source** frame paths - byte for byte the body of the *first* `<format>` in RealityScan's `calibration.xml` (`Image List`, `$(imagePath)$(imageName)$(imageExt)`, `undistortImages="never"`) - with no `images/`, no `sparse/0/` and therefore no mask route at all (7.5's whole delivery is a property of that export). Every step 4 run on that machine had been falling back to `transforms.json` and its one hoisted median intrinsic, which is the exploded-shell failure of 7.2, and the mask run failed its precondition in under a second with nothing on screen to say why. What the app writes was **not** the difference and was checked rather than argued: `colmap_export_params.xml` is byte-identical on both machines (`cmp`, 1529 B), `align.rscmd` differs only in the drive letter of the path it points at, and both installs are RealityScan 2.2.0.119430, read out of the `.rsproj` header on each side. The tell is in `rc_progress.txt`, which already had it: the second `-exportRegistration` is task `20576` a second time, and it ran for **0.01 s over 2 progress lines** on staging against **10.47 s over 93** on this machine - RS dispatched the export and it returned instantly, having undistorted nothing. So `rc_export_params.check_format_registered` reads the `<format>` table out of `calibration.xml` beside the configured exe and answers whether `{280B11A4-...}` / `RealityScan.Export.COLMAP` is registered at all, before `-align` rather than an hour after it. **It is logged in both directions**, INFO when registered and WARNING when not, for the reason the region's frame check is logged even when it is right (7.4): a check that only speaks on failure cannot be told apart from a check that is not running, and that ambiguity cost a full round trip to the staging PC. And `check_colmap_export` now persists its whole report - counts, model type, registry answer - to `rc_output/colmap_check.json`, beside `alignment_check.json` and for the same reason (7.1): the answer to "did the export this step exists for actually happen" must outlive the scrollback that announced it. **The check refuted the hypothesis it was built to test**, which is why it is worth keeping: staging answers `registered: true, writer RealityScan.Export.COLMAP`, so the format table was never the fault. The export started working there on a re-run with no change to any file the app writes, so the cause was RealityScan's own state on that machine and it is **unexplained**. One candidate is now closed for good, in one line, on every run. |
