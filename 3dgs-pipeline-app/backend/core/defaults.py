...
class ColmapExportDefaults(BaseModel):
    ...
    # Off for an alignment run, and turned on by the mask run (`rc.masks`,
    # TODO P4): what it exports is the *project's* mask layers, which exist
    # only after `-generateMaskFromMesh` has made them. That is the whole mask
    # route — the masks come out under `masks/`, undistorted by the same block
    # as `images/` and named identically, so nothing has to pair them
    # afterwards (step_masks.py).
    export_masks: bool = False
    # `CME_EXT` writes `masks/00000.png`, `CME_MASK_EXT` writes
    # `masks/00000.mask.png`. Only the first is a name LichtFeld Studio pairs
    # with `images/00000.png`; the second is RealityScan's own mask-layer
    # convention and means nothing to LFS. Measured on publicsemple_truck:
    # `ext` gave 251 masks matching 251 images name for name.
    mask_extension: Literal["ext", "mask_ext"] = "ext"
...
class MaskDefaults(BaseModel):
    """Masks made by RealityScan itself, from the region and a mesh (TODO P4).

    A second RealityScan process, not a longer step 3: the mesh is minutes and
    nobody wants to re-align to change a mask. It reopens the saved `.rsproj`,
    sets the region the user placed, calculates a mesh, renders each camera's
    view of it, and **re-runs the COLMAP export with `colmapExportMasks` on**
    (`step_masks.py`).

    That last part is the whole design. `-exportMapsAndMask`, the verb TODO P4
    named, answers *"Feature not implemented"* on RealityScan 2.2 — measured,
    it writes `imageList.txt` and then fails. The COLMAP exporter writes the
    masks itself, through the same undistortion block and the same naming as
    `images/`, so the two sets match by construction and nothing downstream has
    to pair them.
    """
    # Off by default: a mesh inside step 3 is minutes, not seconds, and a
    # project that does not need masks must not pay for them.
    enabled: bool = False
    # `-calculatePreviewModel` / `-calculateNormalModel` / `-calculateHighModel`.
    # Preview is what a *silhouette* needs — the mask is the mesh's outline as
    # seen from the camera, not its surface detail. Measured on
    # publicsemple_truck (251 images): preview mesh 2.3 s, masks 33 s, export
    # 4.3 s. `high` on a 300-image 4K project is not minutes.
    mesh_quality: Literal["preview", "normal", "high"] = "preview"
    # `-setReconstructionRegion <region>/region.rsbox` when the user placed one
    # (CLAUDE.md §7.4). Off sends nothing and RS reconstructs inside whatever
    # region the saved project already carries.
    use_region: bool = True
    # `-save` after the masks are generated, so the mesh and the mask layers
    # survive and a re-export costs the export alone.
    save_project_after: bool = True
    # `-set` keys, from Help/en-US/tutorials/setkeyvaluetable.htm, "Image depth
    # map calculation". They are RS's own defaults; they change how long the
    # mesh takes, and — measured — they do *not* change the resolution the
    # masks come out at, which is half the image either way (step_masks.py).
    ...
    gpu_acceleration: bool = True   # MvsGeometryGpuAccel       (RS default true)
...
class RCDefaults(BaseModel):
    ...
    # Masks generated by RealityScan from the region and a mesh (TODO P4).
    # A separate run (`POST /api/pipeline/masks`), never part of the alignment.
    masks: MaskDefaults = Field(default_factory=MaskDefaults)
